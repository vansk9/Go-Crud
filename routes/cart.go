...
// Setup route untuk cart
func SetupCartRoutes(app *fiber.App) {
	cartGroup := app.Group("/cart", middleware.AuthMiddleware)

	cartGroup.Get("/", GetCartItems)	// Lihat semua item di keranjang
	cartGroup.Get("/:id", GetCartItem)	// Lihat satu item di keranjang
	cartGroup.Post("/", AddCartItem)	// Tambah satu item ke keranjang
	cartGroup.Post("/bulk", AddMultipleCartItems)	// Tambah beberapa item ke keranjang sekaligus
	cartGroup.Put("/:id", UpdateCartItem)	// Update item di keranjang
	cartGroup.Delete("/:id", DeleteCartItem)	// Hapus satu item dari keranjang
	cartGroup.Delete("/bulk", DeleteMultipleCartItems)	// Hapus beberapa item sekaligus
}
